...
(venv) dongwoo@dongwoo:~/iot_ids_project/src$ cat seed_runner.py
import time
import numpy as np
import torch
...
def main():
    ...
    for seed in SEEDS:
        ...
        elif ATTACK_MODE == "unconstrained":
            y_true, y_pred = evaluate_adversarial(
                model, test_loader, device, criterion, EPSILON, mask=None
            )
        elif ATTACK_MODE == "constrained":
            y_true, y_pred = evaluate_adversarial(
                model, test_loader, device, criterion, EPSILON, mask=constrained_mask
            )
        else:
            raise ValueError("ATTACK_MODE must be 'clean', 'unconstrained', or 'constrained'")
    ...
    print("\n===== FINAL SUMMARY =====")
    print(f"Condition: model={MODEL_TYPE}, mode={ATTACK_MODE}, epsilon={EPSILON}")
