...
def pgd_attack(model, criterion, x, y, epsilon, alpha, num_steps, mask=None):
...
def evaluate_pgd(model, loader, device, criterion, epsilon, alpha, num_steps, mask=None):
    model.eval()
    y_true_list = []
    y_pred_list = []
    ...
    for x, y in loader:
        ...
        x_adv = pgd_attack(model, criterion, x, y, epsilon, alpha, num_steps, mask=mask)
...
def main():
    ...
    model = CNN1D(n_features=46).to(device)
    ...
    y_true_un, y_pred_un = evaluate_pgd(
        model, test_loader, device, criterion,
        epsilon=EPSILON, alpha=ALPHA, num_steps=NUM_STEPS, mask=None
    )
    print_metrics("UNCONSTRAINED PGD", y_true_un, y_pred_un)

    y_true_con, y_pred_con = evaluate_pgd(
        model, test_loader, device, criterion,
        epsilon=EPSILON, alpha=ALPHA, num_steps=NUM_STEPS, mask=constrained_mask
    )
    print_metrics("CONSTRAINED PGD", y_true_con, y_pred_con)
